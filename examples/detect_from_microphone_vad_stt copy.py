...
parser.add_argument("--doa_block_confidence_threshold", type=float, default=0.08)
parser.add_argument("--doa_pair_peak_margin_weight", type=float, default=1.0)
parser.add_argument("--doa_pair_peak_ratio_weight", type=float, default=0.7)
parser.add_argument("--doa_block_peak_ratio_weight", type=float, default=0.8)
...
DOA_BLOCK_CONFIDENCE_THRESHOLD = args.doa_block_confidence_threshold
DOA_PAIR_PEAK_MARGIN_WEIGHT = args.doa_pair_peak_margin_weight
DOA_PAIR_PEAK_RATIO_WEIGHT = args.doa_pair_peak_ratio_weight
DOA_BLOCK_PEAK_RATIO_WEIGHT = args.doa_block_peak_ratio_weight
...
def circular_distance_deg(a: float, b: float, n_angles: int = 360) -> float:
    diff = abs(a - b) % n_angles
    return min(diff, n_angles - diff)


def top2_stats(score: np.ndarray):
    if score.size < 2:
        peak = float(np.max(score))
        return peak, peak
    idx = np.argpartition(score, -2)[-2:]
    top2 = np.sort(score[idx])
    return float(top2[-1]), float(top2[-2])


def pair_reliability_from_score(score: np.ndarray) -> float:
    peak, second = top2_stats(score)
    median = float(np.median(score))
    peak_margin = max(peak - second, 0.0)
    peak_ratio = peak / max(second, 1e-6)
    prominence = max(peak - median, 0.0)
    reliability = (
        DOA_PAIR_PEAK_MARGIN_WEIGHT * peak_margin
        + DOA_PAIR_PEAK_RATIO_WEIGHT * max(peak_ratio - 1.0, 0.0)
        + 0.3 * prominence
    )
    return max(reliability, 0.0)


def block_confidence_from_prob(prob: np.ndarray) -> float:
    peak, second = top2_stats(prob)
    peak_ratio = peak / max(second, 1e-6)
    concentration = max(peak - float(np.mean(prob)), 0.0)
    return max(concentration + DOA_BLOCK_PEAK_RATIO_WEIGHT * max(peak_ratio - 1.0, 0.0), 0.0)
...
class DOAProbabilityEstimator:
    ...
    def process_block(self, raw4: np.ndarray):
        ...
        assert raw4.shape[0] == self.block_size
        ...
        for pair_idx, (i, j) in enumerate(MIC_PAIRS):
            cc, tau_axis = gcc_phat(
                ...
                interp=DOA_INTERP,
                low_freq_hz=DOA_LOW_FREQ_HZ,
                high_freq_hz=DOA_HIGH_FREQ_HZ,
            )
            ...
            pair_weights.append(pair_reliability_from_score(score))
        ...
        if pair_weight_sum > DOA_EPS:
            pair_weights = pair_weights / pair_weight_sum
            score = np.sum(pair_scores * pair_weights[:, None], axis=0)
        else:
        ...
        self.prev_prob = prob
        block_confidence = block_confidence_from_prob(prob)
        return {
            "prob": prob,
...
def estimate_doa_from_multichannel_audio(raw_audio_int16: np.ndarray):
    ...
    for start in range(0, raw_audio_int16.shape[0], args.doa_block_size):
        ...
        block_result = estimator.process_block(block)
        if block_result is not None and block_result["block_confidence"] >= DOA_BLOCK_CONFIDENCE_THRESHOLD:
            probs.append(block_result["prob"])
            block_confidences.append(block_result["block_confidence"])
            peak_angles.append(int(np.argmax(block_result["prob"])))

    if not probs:
        return None

    probs_arr = np.stack(probs, axis=0)
    conf_arr = np.asarray(block_confidences, dtype=np.float64)
    conf_arr = conf_arr / max(np.sum(conf_arr), DOA_EPS)
    mean_prob = np.sum(probs_arr * conf_arr[:, None], axis=0)
    mean_prob = mean_prob / np.sum(mean_prob)
    peak_raw = int(np.argmax(mean_prob))
    peak_calib = (peak_raw + args.doa_angle_offset_deg) % args.doa_n_angles
    consistent_blocks = sum(
        1 for angle in peak_angles if circular_distance_deg(angle, peak_raw, args.doa_n_angles) <= 15
    )
    return {
        "prob": mean_prob,
        "peak_raw": peak_raw,
        ...
        "mean_block_confidence": float(np.mean(block_confidences)),
        "consistent_blocks": int(consistent_blocks),
    }
...
def finalize_command_and_return_to_wakeword():
    ...
    if speech_len_sec >= MIN_VALID_SPEECH_SECONDS:
        if args.save_debug_audio:
            ...
            save_multichannel_wav(args.save_multichannel_path, command_raw_audio, RATE)
            print(f"[INFO] Saved 4-channel command audio to {args.save_multichannel_path}")

        save_wav(args.save_path, processed_audio, RATE)
        print(f"[INFO] Saved speech-only command audio to {args.save_path}")

        doa_result = estimate_doa_from_multichannel_audio(command_raw_audio)
        if doa_result is not None:
            print(
                ...
                f"mean_block_conf={doa_result['mean_block_confidence']:.3f}, "
                f"consistent_blocks={doa_result['consistent_blocks']})"
            )
        else:
            print("[DOA] Not enough valid multi-channel speech energy for angle estimation.")
